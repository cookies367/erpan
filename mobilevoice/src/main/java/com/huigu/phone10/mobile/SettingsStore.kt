...
/** 语音服务预设：保存一套地址/Key/模型/音色，方便一键切换 */
    ...
    fun load(): MobileSettings {
        ...
        try {
            ...
            return gson.fromJson(cipher.doFinal(bytes.copyOfRange(12, bytes.size)).toString(Charsets.UTF_8), MobileSettings::class.java)
                // 旧版配置没有 sttPresets/ttsPresets 字段，Gson 会给 null，这里兜底
                .let { it.copy(sttPresets = it.sttPresets ?: emptyList(), ttsPresets = it.ttsPresets ?: emptyList()) }
        } catch (_: Exception) {
